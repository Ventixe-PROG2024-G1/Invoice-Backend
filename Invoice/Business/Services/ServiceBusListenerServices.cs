...
public class ServiceBusListenerServices(IOptions<ServiceBusSettings> sbOptions, IServiceScopeFactory scopeFactory, ILogger<ServiceBusListenerServices> logger) : BackgroundService
{
    private readonly ServiceBusProcessor _processor
        = new ServiceBusClient(sbOptions.Value.ConnectionString)
            .CreateProcessor(
                topicName: sbOptions.Value.TopicName,
                subscriptionName: sbOptions.Value.SubscriptionName,
    ...
    private readonly ILogger<ServiceBusListenerServices> _logger = logger;
    ...
    private async Task OnMessageAsync(ProcessMessageEventArgs args)
    {
        ...
        try
        {
            var order = JsonSerializer.Deserialize<BookingTicketOrder>(body, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });

            if (order == null)
            {
                throw new Exception("Kunde inte göras om till BookingTicketOrder");
            }
                return;
